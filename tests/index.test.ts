...
function wrapper(config: { baseUrl?: string } = {}) {
	return ({ children }: { children: ReactNode }) =>
		createElement(CmsProvider, { config: { baseUrl: config.baseUrl ?? 'http://localhost:3001' } }, children);
}

describe('CmsProvider', () => {
	it('provides context to children', () => {
		const { result } = renderHook(() => useQuery(c => c.get('/test')), { wrapper: wrapper() });
...
describe('useQuery', () => {
	...
	it('throws without CmsProvider', () => {
		expect(() => renderHook(() => useQuery(c => c.get('/test'))))
			.toThrow('CmsProvider must wrap your component tree');
	});
});
...
describe('useMutation', () => {
	beforeEach(() => {
		vi.stubGlobal('fetch', vi.fn(async (_url: string, init?: RequestInit) => {
			...
			return new Response(JSON.stringify({ id: 'new-1', ...body }), { status: 201, headers: { 'content-type': 'application/json' } });
		}));
	});
	...
	it('starts not loading', () => {
		const { result } = renderHook(
			() => useMutation((c, v: { name: string }) => c.post<{ id: string }>('/items', v)),
			{ wrapper: wrapper() }
		);
